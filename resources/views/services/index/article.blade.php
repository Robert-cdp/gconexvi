    ...
    <article class="bg-white rounded-2xl overflow-hidden border border-slate-200 card-hover shadow-sm flex flex-col">
        <div class="relative">
            <img src="{{ Storage::url($service->cover->path) }}" class="h-44 w-full object-cover"
                alt="{{ $service->title }}">
            {{-- <span class="absolute top-3 left-3 bg-primary-600 text-white text-xs px-2.5 py-0.5 rounded-full shadow">Destacado</span> --}}
        </div>
        <div class="p-5 flex flex-col flex-1">
            <div class="flex items-center gap-3 mb-3">
                <img src="{{ Storage::url($service->user->avatar) }}" class="w-10 h-10 rounded-full ring-2 ring-primary-100" alt="{{ $service->user->name }}">
                <div>
