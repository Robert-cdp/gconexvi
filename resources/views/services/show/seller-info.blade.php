 <div class="flex items-center gap-4 p-4 bg-white border border-slate-200 rounded-2xl shadow-sm">
     <img src="{{ $service->user->avatar ? Storage::url($service->user->avatar) : asset('storage/images/default-avatar.webp') }}" alt="{{ $service->user->name }}" class="w-14 h-14 rounded-full ring-2 ring-primary-100 object-cover">
    <div class="flex-1">
